...
export default function App() {
  ...
  async function triggerScan() {
    setScanning(true)
    setScanMsg('⏳ Đang trigger scan trên server...')
    const r = await fetch('/api/scan', { method:'POST' })
    const d = await r.json()
    setScanMsg(d.ok
      ? '✅ Scan đang chạy! Leads sẽ tự động cập nhật sau vài phút.\n\nXem Railway Logs để theo dõi chi tiết.'
      : '⚠️ ' + d.message)
    setScanning(false)
    ...
    try {
      const res = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          system:`Bạn là sales analyst cho Coincu.com. Phân tích notes và tạo daily standup.
    ...
    <div style={{ minHeight:'100vh',background:'#F9FAFB',fontFamily:"'DM Sans','Helvetica Neue',sans-serif" }}>
      ...
      <div style={{ background:'#111',color:'#fff',padding:'0 24px',height:56,display:'flex',alignItems:'center',justifyContent:'space-between',position:'sticky',top:0,zIndex:10 }}>
        <div style={{ display:'flex',alignItems:'center',gap:8 }}>
          ...
          <span style={{ background:'#333',fontSize:11,padding:'2px 8px',borderRadius:4,marginLeft:4 }}>v2.0</span>
        </div>
        ...
        <button onClick={() => setShowApiInput(!showApiInput)} style={{ ...B.s,fontSize:12,padding:'5px 12px',background:apiKey?'#1a1a1a':'#fff',color:apiKey?'#4ade80':'#374151',borderColor:apiKey?'#4ade80':'#D1D5DB' }}>
          {apiKey ? '🔑 Key ✓' : '🔑 API Key'}
        </button>
      </div>
      ...
      <div style={{ padding:24,maxWidth:1080,margin:'0 auto' }}>
        ...
        {tab==='Leads' && (
          <div>
            ...
            <div style={{ display:'flex',flexDirection:'column',gap:10 }}>
              {filtered.map(l => (
                <div key={l.id} style={{ background:'#fff',border:'1px solid #E5E7EB',borderRadius:12,padding:'14px 18px' }}>
            ...
            <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:20 }}>
              ...
              <div>
                <div style={{ display:'flex',justifyContent:'space-between',marginBottom:8 }}>
                  <span style={{ fontSize:13,fontWeight:600,color:'#374151' }}>Output</span>
        ...
        {tab==='Auto Scan' && (
          <div>
            ...
            <p style={{ color:'#6B7280',fontSize:13,marginBottom:20 }}>Hệ thống tự động quét Telegram DM + Lark Mail mỗi 2 giờ → cập nhật status lead</p>
            <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:16,marginBottom:20 }}>
              {[['📱','Telegram DM','Đọc tin nhắn với từng lead','TELEGRAM_SESSION'],['📧','Lark Mail','Đọc email inbox','LARK_APP_ID + SECRET']].map(([icon,title,desc,env]) => (
                <div key={title} style={{ background:'#fff',border:'1px solid #E5E7EB',borderRadius:12,padding:18 }}>
                  <div style={{ fontSize:24,marginBottom:8 }}>{icon}</div>
                  <div style={{ fontWeight:600,marginBottom:4 }}>{title}</div>
                  <div style={{ fontSize:13,color:'#6B7280',marginBottom:8 }}>{desc}</div>
                  <div style={{ fontSize:11,background:'#F3F4F6',padding:'4px 8px',borderRadius:4,fontFamily:'monospace' }}>{env}</div>
                </div>
              ))}
            </div>
            <div style={{ background:'#fff',border:'1px solid #E5E7EB',borderRadius:12,padding:20,marginBottom:16 }}>
              <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12 }}>
                ...
                <button onClick={triggerScan} disabled={scanning} style={{ ...B.p,background:'#7C3AED',opacity:scanning?0.6:1 }}>
                  {scanning ? '⏳ Đang chạy...' : '🔍 Scan Now'}
                </button>
              </div>
              <div style={{ background:'#0D0D0D',color:'#00FF88',padding:16,borderRadius:8,fontFamily:'monospace',fontSize:12,minHeight:120,whiteSpace:'pre-wrap',lineHeight:1.6 }}>
                {scanMsg || 'Bấm "Scan Now" để trigger scan thủ công.\nAuto scan chạy mỗi 2 giờ trên server.\n\nCần set Railway Variables:\n- TELEGRAM_SESSION\n- LARK_APP_ID + LARK_APP_SECRET\n- ANTHROPIC_API_KEY'}
              </div>
            </div>
          </div>
        )}
          ...
          <div>
            ...
            <div style={{ background:'#fff',border:'1px solid #E5E7EB',borderRadius:12,padding:20 }}>
              <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:10 }}>
                {[['name','Tên dự án *'],['website','Website'],['sources','Nguồn (TG group, event...)'],['telegram_username','Telegram username (không có @)'],['lark_email','Lark email của lead'],['note','Ghi chú outreach']].map(([k,p]) => (
              ...
              <button onClick={addLead} disabled={loading} style={{ ...B.p,width:'100%',opacity:loading?0.6:1 }}>
                {loading ? 'Đang thêm...' : '+ Add Lead'}
              </button>
            </div>
          </div>
        )}
